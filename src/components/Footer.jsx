...
export default function Footer({ onCtaClick }) {
  return (
    <footer className="relative bg-void text-ghost pt-24 md:pt-32 pb-12 px-6 md:px-8 overflow-hidden rounded-t-[3rem] md:rounded-t-[4rem] border-t border-plasma/20 mt-[-2rem] md:mt-[-3rem] z-20 shadow-[0_-30px_80px_rgba(10,10,20,1)]">
      ...
      <div className="max-w-7xl mx-auto relative z-10 flex flex-col lg:flex-row justify-between lg:items-end gap-12 lg:gap-16">
        <div className="max-w-xl text-center lg:text-left">
          <p className="font-mono text-[9px] text-plasma/50 uppercase tracking-[0.4em] mb-6">¿Listo para crecer?</p>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif italic text-plasma text-glow-plasma mb-8 leading-tight">
            Elimina tus trabas.<br />
            <span className="font-sans font-extrabold not-italic text-ghost tracking-tighter uppercase text-4xl md:text-6xl">Hoy mismo.</span>
          </h2>
          <MagneticButton onClick={onCtaClick} className="px-10 md:px-12 py-3.5 md:py-4 text-sm md:text-base w-full md:w-auto">
            ¡Quiero eliminar mis trabas ahora!
          </MagneticButton>
        </div>

        <div className="flex flex-col lg:items-end gap-10 lg:gap-12 w-full lg:w-1/2">
          <div className="flex items-center gap-3 bg-graphite/60 px-6 py-3 rounded-full border border-ghost/10 self-center lg:self-end">
          ...
          <div className="max-w-full lg:max-w-[450px] font-sans text-center lg:text-right">
            ...
            <p className="text-[#a0a0a0] text-[11px] md:text-xs leading-relaxed">
