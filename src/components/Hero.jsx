...
export default function Hero({ onCtaClick }) {
  ...
  return (
    <section
      ref={containerRef}
      className="relative h-[100dvh] w-full flex flex-col justify-end pb-24 px-8 md:px-16 overflow-hidden"
    >
      {/* Background */}
      <div className="absolute inset-0 z-0 bg-void">
        <img
          ...
          className="w-full h-full object-cover opacity-40 saturate-50 mix-blend-screen mix-blend-lighten"
        />
      ...
      {/* Orbs */}
      <div className="hero-orb absolute top-[25%] left-[15%] w-64 h-64 rounded-full bg-plasma/10 blur-[90px] pointer-events-none mix-blend-screen" />
      <div className="hero-orb absolute top-[40%] right-[10%] w-96 h-96 rounded-full bg-plasma/10 blur-[120px] pointer-events-none mix-blend-screen" />

      {/* Content */}
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-12">
        <div className="flex-1">
          <div className="hero-line w-20 h-px bg-plasma mb-8" />
          <p className="hero-text font-mono text-xs tracking-[0.4em] text-plasma uppercase mb-6">
            Agencia de Automatizaciones · IA Estratégica
          </p>
          <h1 className="hero-text font-sans font-extrabold text-5xl md:text-[7.5rem] leading-[0.88] tracking-tighter text-ghost uppercase">
            Si es repetitivo,<br />
            <span className="font-serif italic font-normal text-plasma lowercase text-glow-plasma">
              lo automatizamos.
            </span>
          </h1>
          <h2 className="hero-text font-sans font-extrabold text-5xl md:text-[7.5rem] leading-[0.88] tracking-tighter text-ghost/30 uppercase mt-2">
            Si es un obstáculo,<br />
            <span className="font-serif italic font-normal text-ghost/20 lowercase">lo eliminamos.</span>
          </h2>
        </div>
        <div className="hero-text md:w-[400px] flex flex-col gap-7 md:pb-10">
          <p className="font-sans text-base md:text-lg text-ghost/75 leading-relaxed">
          ...
          <MagneticButton onClick={onCtaClick} className="self-start text-sm px-8 py-4 w-full text-center">
            ¡Quiero eliminar mis trabas ahora!
          </MagneticButton>
          <p className="font-mono text-xs text-ghost/40 leading-relaxed">
            Agenda una consultoría ágil y descubre qué<br />
